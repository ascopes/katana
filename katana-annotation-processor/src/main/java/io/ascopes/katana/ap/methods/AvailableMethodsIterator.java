...
public final class AvailableMethodsIterator extends KatanaIterator<ExecutableElement> {
  ...
  private Stream<ExecutableElement> visibleSignaturesForList(
    ...
    Map<ExecutableType, List<AnnotationMirror>> allAnnotations = new HashMap<>();
    ...
    for (ExecutableElement nextSignature : signatures) {
      ...
      for (ExecutableElement seenSignature : seenSignatures) {
        ...
        // TODO(ascopes): verify I have this the right way around.
        if (this.typeUtils.isSubsignature(seenSignatureType, nextSignatureType)) {
          continue outerLoop;
        }
      }
      ...
      allAnnotations
          .computeIfAbsent(nextSignatureType, unused -> new ArrayList<>())
          .addAll(nextSignatureType.getAnnotationMirrors());
    }
